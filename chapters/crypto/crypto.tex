\chapter{From digital cash to cryptoeconomical distributed ledgers}
\label{sec:crypto}
...
The most important op codes are related to cryptographic primitives such as digital signature verification and secure hash functions. Bitcoin uses an elliptic curve digital signature algorithm (ECDSA) for authentication. In the prevailing case a derivation of an ECDSA public key is identified with a Bitcoin address, and the corresponding private key is used to create a signature to authenticate the spending of an UTXO corresponding to this Bitcoin address. 
...
Bitcoin, i.e. the reference implementation Bitcoin core, currently distinguishes five types of pubScripts:
...
If a pubScript can not be categorized in one of these types, it is termed \emph{non-standard}. Bitcoin core nodes will not relay transactions containing non-standard pubScripts. However, if a non-standard transaction ends up in the blockchain because a miner  (see Sec. \ref{sec:mining}) has a different policy, Bitcoin core nodes will still accept it.
...
\subsection{OPTIONAL: Decentralized Exchanges}
